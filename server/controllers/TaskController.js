import { selectAllTasks, insertTask, removeTask } from '../models/Task.js'
import { ApiError } from '../helpers/Apierror.js'

const getTasks = async (req, res, next) => {
    try {
        const result = await selectAllTasks(req.user)
        return res.status(200).json(result.rows || [])
    } catch (error) {
        return next(new ApiError('Failed to get tasks', 500))
    }
}

const postTask = async (req, res, next) => {
    const { task } = req.body
    try {
        if (!task || !task.description || task.description.trim().length === 0) {
            return next(new ApiError('Task description is required', 400))
        }
        const result = await insertTask(task.description, req.user)
        return res.status(201).json({ id: result.rows[0].id, description: result.rows[0].description })
    } catch (error) {
        return next(new ApiError('Failed to create task', 500))
    }
}

const deleteTask = async (req, res, next) => {
    const { id } = req.params
    try {
        const result = await removeTask(id, req.user)
        if (result.rowCount === 0) {
            return next(new ApiError('Task not found', 404))
        }
        return res.status(200).json({ id: id })
    } catch (error) {
        return next(new ApiError('Failed to delete task', 500))
    }
}

export { getTasks, postTask, deleteTask }
